...
def configure_and_run_lumerical(charge_file):
    ...
    logging.debug("creating directory for file: %s", filename)
    os.mkdir(filename)
    shutil.move(charge_file, "./{}/{}.mat".format(filename, filename))
    os.chdir(filename)
    out_file = "{}.ldev".format(filename)

    logging.debug("Running lumerical on %s", charge_file)
    ...
    # Clean up directory
    if PURGE_LARGE_DATA:
        logging.info("Purging large data files")
...
for f in dirlist:
    if ".lsf" in f:
        logger.info("found .lsf file '%s'", f)
